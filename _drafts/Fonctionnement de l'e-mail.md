...
Le symbole @ permet de cibler les messages vers un seul utilisateur sur une machine spécifique, plutôt que vers un localhost (l'ordinateur joue le rôle de serveur virtuel). 

Après le développement du DNS (système de noms de domaine), les adresses e-mail étaient au format loginname@host.domain.

Pour la petite histoire, le **courrier électronique** ne s’appelait pas encore e-mail mais Netmail pour Network Mail. 

7 ans plus tard, en mai 1978, Gary Thuerk a envoyé  ce qui est considéré comme étant le premier **spam** de l'histoire à près de 600 personnes.

Vous pouvez retrouver la reproduction de celui-ci sur [le blog de Brad Templetons](https://www.templetons.com/brad/spamreact.html), l’un des pionniers du web.  
...
- 1996 : Création d’Hotmail, l’un des pionniers dans la messagerie “gratuite” en ligne. Le courrier électronique dépasse le courrier postal. 

- 2004 : Lancement de Gmail 

- 2011 : 3,1 milliards d’adresses mails dans le monde sont recensées.

- 2012 : l’e-mail devient le mode de communication principal sur smartphone grâce à l’arrivée du Responsive Design 
...
- **Mondiale** : Les e-mails peuvent être envoyés dans presque tous les pays depuis n'importe où dans le monde.

-  **Interopérable** : un e-mail peut être instantanément envoyé et reçu par le destinataire en temps réel via Internet tout en étant compatible avec différents systèmes d'information.
...
**Le webmail** et le client de messagerie remplissent la même fonction : ils permettent aux utilisateurs d'envoyer et de recevoir des e-mails. Cependant, un **client de messagerie** exige que les utilisateurs installent un logiciel  directement sur leur ordinateur ou via une application dédiée ; si le logiciel / l’application n’est pas installé, l'accès à la messagerie peut ne pas être accessible. 

En outre, de nombreux clients de messagerie coûtent de l'argent, mais sont généralement plus sécurisés. D'autre part, le Webmail  est un service gratuit, hébergé dans le **cloud** qui synchronise automatiquement votre e-mail sur tous vos appareils (ordinateur, smartphone, tablette). Il est accessible sur n’importe quel navigateur web comme Firefox ou Duck Duck Go. 

**Avantages du webmail** : 

* **Accessible** depuis n’importe quel appareil avec un navigateur internet 
* **Plus rapide** (pas besoin de télécharger un logiciel qui doit être installé et configuré localement)
* **Aucune mise à jour**  à effectuer 
* Basé sur le protocole **IMAP** 

Le **client de messagerie** est un logiciel qui doit être installé, vos e-mails sont téléchargés à partir d'un serveur spécifique et stockés localement ou en ligne via le protocole IMAP ou vous pouvez via le **protocole POP3** consulter vos mails uniquement en hors-ligne sur l’appareil où vous avez installé votre programme de messagerie. 

**Avantages des clients de messagerie** : 

* Plus d’espace de **stockage** 
* Plus de **fonctionnalités**
* D’autres services de **communication intégrés**
* …
...
* **MUA** (Mail User Agent) : client de messagerie comme  Thundebird, Gmail ou Outlook
