...
* La politique de gestion DMARC d’Emma doit vérifier l’authenticité de l’e-mail pour savoir comment administrer le mail qui arrive et va demander au DNS de Gabriel
...
* DKIM permet d’attester l'intégrité du contenu des e-mails à l'aide d'une signature numérique associée à votre domaine.  DMARC le reconnaît grâce au nom de domaine. Le MTA d’Emma va donc chercher la clé publique de Gabrielle via son DNS pour l’authentifier. Si l’IP coïncide à la liste IP autorisée, le MTA signe le mail en utilisant la clé privée. 

À savoir : la clé publique correspond à la clé privée, c’est une sorte « d’empreinte » . 
